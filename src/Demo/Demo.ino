 ...
 *  - Structured Serial Monitor logs
 */
...
// ─── Supabase Config ─────────────────────────────────────────────────────────
// IMPORTANT: For final submission, it is better to move these to wifi_config.h
...
const unsigned long SEND_INTERVAL      = 10000;  // send/buffer every 10 seconds
...
// ─── POST latest cached values to Supabase ───────────────────────────────────
void postToSupabase() {
  unsigned long now = millis();
...
// ─── POST a single reading to Supabase with retry ────────────────────────────
bool sendToSupabase(const char* sensorId, float temperature, float humidity) {
